...
class ComprehensiveGATModel(nn.Module):
    """Enhanced GAT model for comprehensive data"""
    
    def __init__(self, num_features: int, hidden_dim: int = 128, num_layers: int = 4, 
                 num_heads: int = 8, num_classes: int = 2, dropout: float = 0.3):
        super(ComprehensiveGATModel, self).__init__()
        ...
        # GAT layers with larger capacity for comprehensive data
...
class ComprehensiveGCNModel(nn.Module):
    """Enhanced GCN model for comprehensive data"""
    
    def __init__(self, num_features: int, hidden_dim: int = 128, num_layers: int = 4, 
                 num_classes: int = 2, dropout: float = 0.3):
        super(ComprehensiveGCNModel, self).__init__()
...
class ComprehensiveGraphSAGEModel(nn.Module):
    """Enhanced GraphSAGE model for comprehensive data"""
    
    def __init__(self, num_features: int, hidden_dim: int = 128, num_layers: int = 4, 
                 num_classes: int = 2, dropout: float = 0.3):
        super(ComprehensiveGraphSAGEModel, self).__init__()
...
class ComprehensiveTrainer:
    """Enhanced trainer for comprehensive data"""
    
    def __init__(self, device='cpu'):
        self.device = device
        self.results = {}
        
    def load_comprehensive_data(self, method='raw') -> Data:
        """Load comprehensive data"""
        logger.info(f"Loading comprehensive {method} data...")
        
        comprehensive_dir = Path("data/comprehensive")
        
        try:
            # Load the comprehensive graph and features
            with open(comprehensive_dir / "comprehensive_graph.pkl", 'rb') as f:
                graph = pickle.load(f)
            
            with open(comprehensive_dir / "node_features.pkl", 'rb') as f:
                node_features = pickle.load(f)
            
            with open(comprehensive_dir / "labels.pkl", 'rb') as f:
            ...
            # Create edge index from comprehensive graph
            edge_list = []
            ...
            logger.info(f"Loaded comprehensive {method} data: {data.x.shape}, {data.edge_index.shape}, {data.y.shape}")
    ...
    def run_comprehensive_evaluation(self, model_class, model_name: str, data: Data, 
                                   loss_type: str = 'focal') -> Dict:
        """Run comprehensive evaluation with comprehensive data"""
        ...
        # Enhanced hyperparameters for comprehensive data
        best_params = {
            'hidden_dim': 128,
            'num_layers': 4,
            'dropout': 0.3,
    ...
    def save_results(self, results: Dict, method: str):
        ...
        # Save JSON results
        filename = f"{results['model_name']}_comprehensive_{method}_{results['loss_type']}_results.json"
        with open(f'results/{filename}', 'w') as f:
            json.dump(json_results, f, indent=2)
        
        # Save detailed metrics
        metrics_file = f'results/{results["model_name"]}_comprehensive_{method}_{results["loss_type"]}_metrics.json'
...
def main():
    ...
    logger.info("Starting comprehensive training with ALL data...")
    
    # Initialize trainer
    trainer = ComprehensiveTrainer(device='cpu')
    
    # Define models
    models = {
        'GAT': ComprehensiveGATModel,
        'GCN': ComprehensiveGCNModel,
        'GraphSAGE': ComprehensiveGraphSAGEModel
    }
    
    # Load comprehensive raw data
    logger.info(f"\n{'='*60}")
    logger.info(f"Testing COMPREHENSIVE RAW DATASET")
    logger.info(f"{'='*60}")
    
    # Load comprehensive data
    data = trainer.load_comprehensive_data('raw')
    
    if data is None:
        logger.error("Failed to load comprehensive data. Exiting.")
        return
    
    # Check if we have multiple classes
    unique_labels = torch.unique(data.y)
    if len(unique_labels) < 2:
        logger.warning(f"Only {len(unique_labels)} class(es) found. Creating synthetic labels for demonstration.")
        # Create synthetic labels for demonstration (top 10% as positive class)
        num_nodes = data.y.size(0)
        num_positive = num_nodes // 10
        synthetic_labels = torch.zeros(num_nodes, dtype=torch.long)
        synthetic_labels[:num_positive] = 1
        data.y = synthetic_labels
        logger.info(f"Created synthetic labels: {dict(Counter(data.y.numpy()))}")
    ...
    for loss_type in loss_types:
        ...
        for model_name, model_class in models.items():
            logger.info(f"\nTraining {model_name} with comprehensive raw data and {loss_type} loss...")
            ...
            # Store results
            key = f"{model_name}_comprehensive_raw_{loss_type}"
            all_results[key] = results
            
            # Save results
            trainer.save_results(results, 'raw')
    
    # Print final comparison
    logger.info(f"\n{'='*100}")
    logger.info("FINAL COMPREHENSIVE RESULTS COMPARISON")
    logger.info(f"{'='*100}")
    
    for key, results in all_results.items():
        metrics = results['average_metrics']
        logger.info(f"{key:40} | F1: {metrics['f1']:.4f} | Balanced Acc: {metrics['balanced_accuracy']:.4f} | "
