import os
import requests
import pandas as pd
import numpy as np
import json
import logging
import time
from pathlib import Path
from typing import Dict, List, Optional, Tuple
import gzip
import tarfile
from urllib.parse import urljoin
import warnings
warnings.filterwarnings('ignore')

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

class RealClinicalDataIntegrator:
    """
    Real clinical data integrator that connects to actual TCGA and CPTAC APIs
    to fetch real patient data for cancer genomics analysis
    """
    
    def __init__(self, output_dir: str = "data/real_clinical"):
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        # Create subdirectories
        (self.output_dir / "mutations").mkdir(exist_ok=True)
        (self.output_dir / "expression").mkdir(exist_ok=True)
        (self.output_dir / "clinical").mkdir(exist_ok=True)
        (self.output_dir / "protein").mkdir(exist_ok=True)
        (self.output_dir / "cnv").mkdir(exist_ok=True)
        
        # API endpoints
        self.gdc_api_base = "https://api.gdc.cancer.gov"
        self.tcga_data_portal = "https://portal.gdc.cancer.gov"
        self.cptac_data_portal = "https://cptac-data-portal.georgetown.edu"
        
        # Session for API calls
        self.session = requests.Session()
        self.session.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
        })
        
        # Cancer types to fetch
        self.cancer_types = ['BRCA', 'LUAD', 'LUSC', 'COAD', 'READ', 'STAD', 'LIHC', 'KIRC']
        
        # Data types to fetch
        self.data_types = {
            'mutations': ['Simple Nucleotide Variation'],
            'expression': ['Gene Expression Quantification'],
            'clinical': ['Clinical Supplement'],
            'protein': ['Protein Expression Quantification'],
            'cnv': ['Copy Number Variation']
        }
    
    def fetch_tcga_file_manifest(self, cancer_type: str, data_type: str) -> List[Dict]:
        """
        Fetch file manifest from TCGA GDC API
        """
        logger.info(f"Fetching {data_type} manifest for {cancer_type}")
        
        # GDC API query
        query = {
            "filters": {
                "op": "and",
                "content": [
                    {
                        "op": "=",
                        "content": {
                            "field": "cases.project.project_id",
                            "value": f"TCGA-{cancer_type}"
                        }
                    },
                    {
                        "op": "=",
                        "content": {
                            "field": "files.data_type",
                            "value": data_type
                        }
                    },
                    {
                        "op": "=",
                        "content": {
                            "field": "files.experimental_strategy",
                            "value": "WXS" if data_type == "Simple Nucleotide Variation" else "RNA-Seq"
                        }
                    }
                ]
            },
            "format": "JSON",
            "size": 1000
        }
        
        try:
            response = self.session.post(
                f"{self.gdc_api_base}/files",
                json=query,
                timeout=30
            )
            response.raise_for_status()
            
            data = response.json()
            files = data.get('data', {}).get('hits', [])
            
            logger.info(f"Found {len(files)} {data_type} files for {cancer_type}")
            return files
            
        except Exception as e:
            logger.error(f"Error fetching manifest for {cancer_type} {data_type}: {e}")
            return []
    
    def download_tcga_file(self, file_id: str, file_name: str, data_type: str) -> Optional[str]:
        """
        Download a single file from TCGA GDC
        """
        try:
            # Get file download URL
            response = self.session.get(f"{self.gdc_api_base}/data/{file_id}")
            response.raise_for_status()
            
            # Save file
            output_path = self.output_dir / data_type / f"{file_name}.tsv"
            
            with open(output_path, 'wb') as f:
                f.write(response.content)
            
            logger.info(f"Downloaded {file_name} to {output_path}")
            return str(output_path)
            
        except Exception as e:
            logger.error(f"Error downloading {file_id}: {e}")
            return None
    
    def fetch_tcga_mutation_data(self, cancer_type: str, max_files: int = 50) -> List[str]:
        """
        Fetch real mutation data from TCGA
        """
        logger.info(f"Fetching real mutation data for {cancer_type}")
        
        # Get file manifest
        files = self.fetch_tcga_file_manifest(cancer_type, "Simple Nucleotide Variation")
        
        if not files:
            logger.warning(f"No mutation files found for {cancer_type}")
            return []
        
        # Download files (limit for demonstration)
        downloaded_files = []
        for i, file_info in enumerate(files[:max_files]):
            file_id = file_info['file_id']
            file_name = file_info['file_name']
            
            logger.info(f"Downloading mutation file {i+1}/{min(len(files), max_files)}: {file_name}")
            
            file_path = self.download_tcga_file(file_id, f"{cancer_type}_mutation_{i}", "mutations")
            if file_path:
                downloaded_files.append(file_path)
            
            time.sleep(1)  # Rate limiting
        
        return downloaded_files
    
    def fetch_tcga_expression_data(self, cancer_type: str, max_files: int = 50) -> List[str]:
        """
        Fetch real expression data from TCGA
        """
        logger.info(f"Fetching real expression data for {cancer_type}")
        
        # Get file manifest
        files = self.fetch_tcga_file_manifest(cancer_type, "Gene Expression Quantification")
        
        if not files:
            logger.warning(f"No expression files found for {cancer_type}")
            return []
        
        # Download files
        downloaded_files = []
        for i, file_info in enumerate(files[:max_files]):
            file_id = file_info['file_id']
            file_name = file_info['file_name']
            
            logger.info(f"Downloading expression file {i+1}/{min(len(files), max_files)}: {file_name}")
            
            file_path = self.download_tcga_file(file_id, f"{cancer_type}_expression_{i}", "expression")
            if file_path:
                downloaded_files.append(file_path)
            
            time.sleep(1)  # Rate limiting
        
        return downloaded_files
    
    def fetch_tcga_clinical_data(self, cancer_type: str) -> List[str]:
        """
        Fetch real clinical data from TCGA
        """
        logger.info(f"Fetching real clinical data for {cancer_type}")
        
        # Get file manifest
        files = self.fetch_tcga_file_manifest(cancer_type, "Clinical Supplement")
        
        if not files:
            logger.warning(f"No clinical files found for {cancer_type}")
            return []
        
        # Download files
        downloaded_files = []
        for i, file_info in enumerate(files[:10]):  # Limit clinical files
            file_id = file_info['file_id']
            file_name = file_info['file_name']
            
            logger.info(f"Downloading clinical file {i+1}/{min(len(files), 10)}: {file_name}")
            
            file_path = self.download_tcga_file(file_id, f"{cancer_type}_clinical_{i}", "clinical")
            if file_path:
                downloaded_files.append(file_path)
            
            time.sleep(1)  # Rate limiting
        
        return downloaded_files
    
    def fetch_cptac_protein_data(self, cancer_type: str) -> List[str]:
        """
        Fetch real protein data from CPTAC
        """
        logger.info(f"Fetching real protein data for {cancer_type}")
        
        # CPTAC data URLs (these would need to be updated with actual URLs)
        cptac_urls = {
            'BRCA': {
                'protein': 'https://cptac-data-portal.georgetown.edu/cptac/data/BRCA/Proteome',
                'clinical': 'https://cptac-data-portal.georgetown.edu/cptac/data/BRCA/Clinical'
            },
            'COAD': {
                'protein': 'https://cptac-data-portal.georgetown.edu/cptac/data/COAD/Proteome',
                'clinical': 'https://cptac-data-portal.georgetown.edu/cptac/data/COAD/Clinical'
            }
        }
        
        if cancer_type not in cptac_urls:
            logger.warning(f"No CPTAC URLs configured for {cancer_type}")
            return []
        
        downloaded_files = []
        
        try:
            # Download protein data
            protein_url = cptac_urls[cancer_type]['protein']
            response = self.session.get(protein_url, timeout=30)
            
            if response.status_code == 200:
                output_path = self.output_dir / "protein" / f"{cancer_type}_cptac_protein.tsv"
                with open(output_path, 'wb') as f:
                    f.write(response.content)
                downloaded_files.append(str(output_path))
                logger.info(f"Downloaded CPTAC protein data for {cancer_type}")
            else:
                logger.warning(f"Could not download CPTAC protein data for {cancer_type}")
        
        except Exception as e:
            logger.error(f"Error downloading CPTAC protein data for {cancer_type}: {e}")
        
        return downloaded_files
    
    def fetch_string_ppi_network(self, genes: List[str] = None) -> str:
        """
        Fetch real PPI network from STRING database
        """
        logger.info("Fetching real PPI network from STRING")
        
        if genes is None:
            # Common cancer genes
            genes = [
                'TP53', 'BRCA1', 'BRCA2', 'PTEN', 'PIK3CA', 'KRAS', 'NRAS', 'BRAF',
                'EGFR', 'ERBB2', 'MYC', 'CDKN2A', 'RB1', 'APC', 'VHL', 'NF1',
                'ATM', 'CHEK2', 'PALB2', 'BARD1', 'RAD51C', 'RAD51D', 'BRIP1',
                'CDH1', 'STK11', 'SMAD4', 'TGFBR2', 'MSH2', 'MLH1', 'MSH6'
            ]
        
        try:
            # STRING API call
            string_url = "https://string-db.org/api/tsv/network"
            
            params = {
                'identifiers': '%0d'.join(genes),
                'species': 9606,  # Human
                'required_score': 700,
                'network_type': 'physical'
            }
            
            response = self.session.get(string_url, params=params, timeout=30)
            response.raise_for_status()
            
            # Save PPI data
            ppi_file = self.output_dir / "string_ppi_network.tsv"
            with open(ppi_file, 'w') as f:
                f.write(response.text)
            
            logger.info(f"Downloaded STRING PPI network with {len(response.text.splitlines())} interactions")
            return str(ppi_file)
            
        except Exception as e:
            logger.error(f"Error fetching STRING PPI data: {e}")
            return None
    
    def fetch_kegg_pathways(self) -> str:
        """
        Fetch real pathway data from KEGG
        """
        logger.info("Fetching real pathway data from KEGG")
        
        try:
            # KEGG pathway data for cancer
            kegg_url = "https://rest.kegg.org/list/pathway/hsa"
            response = self.session.get(kegg_url, timeout=30)
            response.raise_for_status()
            
            # Parse KEGG pathways
            pathways = []
            for line in response.text.strip().split('\n'):
                if 'cancer' in line.lower() or 'tumor' in line.lower():
                    pathway_id = line.split('\t')[0]
                    pathway_name = line.split('\t')[1]
                    pathways.append((pathway_id, pathway_name))
            
            # Save pathway data
            pathway_file = self.output_dir / "kegg_cancer_pathways.tsv"
            with open(pathway_file, 'w') as f:
                f.write("pathway_id\tpathway_name\n")
                for pathway_id, pathway_name in pathways:
                    f.write(f"{pathway_id}\t{pathway_name}\n")
            
            logger.info(f"Downloaded KEGG pathway data with {len(pathways)} cancer pathways")
            return str(pathway_file)
            
        except Exception as e:
            logger.error(f"Error fetching KEGG pathway data: {e}")
            return None
    
    def process_real_mutation_data(self, file_path: str) -> pd.DataFrame:
        """
        Process real mutation data from TCGA MAF files
        """
        try:
            # Read MAF file
            if file_path.endswith('.gz'):
                with gzip.open(file_path, 'rt') as f:
                    df = pd.read_csv(f, sep='\t', comment='#')
            else:
                df = pd.read_csv(file_path, sep='\t', comment='#')
            
            # Extract relevant columns
            required_columns = ['Hugo_Symbol', 'Variant_Classification', 'Variant_Type', 'Tumor_Sample_Barcode']
            available_columns = [col for col in required_columns if col in df.columns]
            
            if len(available_columns) < 3:
                logger.warning(f"Insufficient columns in {file_path}")
                return pd.DataFrame()
            
            # Filter for relevant mutations
            df_filtered = df[available_columns].copy()
            
            # Add patient ID
            if 'Tumor_Sample_Barcode' in df_filtered.columns:
                df_filtered['Patient_ID'] = df_filtered['Tumor_Sample_Barcode'].str[:12]
            
            logger.info(f"Processed {len(df_filtered)} mutations from {file_path}")
            return df_filtered
            
        except Exception as e:
            logger.error(f"Error processing mutation file {file_path}: {e}")
            return pd.DataFrame()
    
    def process_real_expression_data(self, file_path: str) -> pd.DataFrame:
        """
        Process real expression data from TCGA
        """
        try:
            # Read expression file
            df = pd.read_csv(file_path, sep='\t')
            
            # Extract gene expression data
            if 'gene_id' in df.columns and 'tpm' in df.columns:
                df_filtered = df[['gene_id', 'tpm']].copy()
                df_filtered.columns = ['Gene', 'Expression']
                
                # Add patient ID from filename
                patient_id = Path(file_path).stem.split('_')[-1]
                df_filtered['Patient_ID'] = patient_id
                
                logger.info(f"Processed {len(df_filtered)} gene expressions from {file_path}")
                return df_filtered
            else:
                logger.warning(f"Unexpected format in expression file {file_path}")
                return pd.DataFrame()
                
        except Exception as e:
            logger.error(f"Error processing expression file {file_path}: {e}")
            return pd.DataFrame()
    
    def process_real_clinical_data(self, file_path: str) -> pd.DataFrame:
        """
        Process real clinical data from TCGA
        """
        try:
            # Read clinical file
            df = pd.read_csv(file_path, sep='\t')
            
            # Extract relevant clinical features
            clinical_features = [
                'Patient ID', 'Age', 'Gender', 'Tumor Stage', 'Survival Status',
                'Overall Survival (Months)', 'Tumor Size', 'Lymph Node Status'
            ]
            
            available_features = [col for col in clinical_features if col in df.columns]
            
            if len(available_features) < 3:
                logger.warning(f"Insufficient clinical features in {file_path}")
                return pd.DataFrame()
            
            df_filtered = df[available_features].copy()
            
            logger.info(f"Processed {len(df_filtered)} clinical records from {file_path}")
            return df_filtered
            
        except Exception as e:
            logger.error(f"Error processing clinical file {file_path}: {e}")
            return pd.DataFrame()
    
    def create_real_data_summary(self) -> Dict:
        """
        Create a summary of all downloaded real data
        """
        summary = {
            'total_files': 0,
            'data_types': {},
            'cancer_types': set(),
            'patients': set(),
            'mutations': 0,
            'expressions': 0,
            'clinical_records': 0
        }
        
        # Count files by type
        for data_type in ['mutations', 'expression', 'clinical', 'protein', 'cnv']:
            dir_path = self.output_dir / data_type
            if dir_path.exists():
                files = list(dir_path.glob('*.tsv'))
                summary['data_types'][data_type] = len(files)
                summary['total_files'] += len(files)
                
                # Extract cancer types and patients from filenames
                for file in files:
                    filename = file.stem
                    if '_' in filename:
                        cancer_type = filename.split('_')[0]
                        summary['cancer_types'].add(cancer_type)
                        
                        # Extract patient ID if available
                        if 'mutation' in filename or 'expression' in filename:
                            parts = filename.split('_')
                            if len(parts) >= 3:
                                patient_id = parts[-1]
                                summary['patients'].add(patient_id)
        
        summary['cancer_types'] = list(summary['cancer_types'])
        summary['patients'] = list(summary['patients'])
        
        logger.info(f"Real data summary: {summary}")
        return summary
    
    def integrate_all_real_data(self, max_files_per_type: int = 20) -> Dict:
        """
        Integrate all real clinical data from multiple sources
        """
        logger.info("Starting comprehensive real clinical data integration")
        
        results = {
            'mutations': [],
            'expression': [],
            'clinical': [],
            'protein': [],
            'ppi_network': None,
            'pathways': None
        }
        
        # Fetch data for each cancer type
        for cancer_type in self.cancer_types:
            logger.info(f"\n{'='*50}")
            logger.info(f"Processing {cancer_type}")
            logger.info(f"{'='*50}")
            
            # Fetch mutation data
            mutation_files = self.fetch_tcga_mutation_data(cancer_type, max_files_per_type)
            results['mutations'].extend(mutation_files)
            
            # Fetch expression data
            expression_files = self.fetch_tcga_expression_data(cancer_type, max_files_per_type)
            results['expression'].extend(expression_files)
            
            # Fetch clinical data
            clinical_files = self.fetch_tcga_clinical_data(cancer_type)
            results['clinical'].extend(clinical_files)
            
            # Fetch protein data (CPTAC)
            protein_files = self.fetch_cptac_protein_data(cancer_type)
            results['protein'].extend(protein_files)
            
            time.sleep(5)  # Rate limiting between cancer types
        
        # Fetch network data
        logger.info("\nFetching network data...")
        results['ppi_network'] = self.fetch_string_ppi_network()
        results['pathways'] = self.fetch_kegg_pathways()
        
        # Create summary
        summary = self.create_real_data_summary()
        
        # Save summary
        summary_file = self.output_dir / "real_data_summary.json"
        with open(summary_file, 'w') as f:
            json.dump(summary, f, indent=2, default=str)
        
        logger.info(f"\nReal data integration completed!")
        logger.info(f"Total files downloaded: {summary['total_files']}")
        logger.info(f"Cancer types: {summary['cancer_types']}")
        logger.info(f"Estimated patients: {len(summary['patients'])}")
        logger.info(f"Summary saved to: {summary_file}")
        
        return results

def main():
    """Main function to integrate real clinical data"""
    logger.info("Starting real clinical data integration")
    
    integrator = RealClinicalDataIntegrator()
    
    # Integrate all real data
    results = integrator.integrate_all_real_data()
    
    logger.info("Real clinical data integration completed successfully!")
    logger.info(f"Results saved to: {integrator.output_dir}")

if __name__ == "__main__":
    main() 