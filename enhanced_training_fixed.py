...
class EnhancedTrainerFixed:
    """
    Enhanced trainer with fixes for class imbalance and paper methodology compliance:
    - Class weights in loss function
    - Balanced accuracy metrics
    - Proper stratified sampling
    - Paper-compliant training procedure
    """
    ...
    def calculate_class_weights(self, data: Data) -> torch.Tensor:
    ...
    def create_stratified_splits(self, data: Data, n_splits: int = 5) -> List[Tuple]:
        """Create stratified train/val/test splits (70/15/15) with proper stratification"""
    ...
    def train_model(self, model, data: Data, train_idx, val_idx, 
                   ...
                   epochs: int = 100, patience: int = 10, class_weights: torch.Tensor = None) -> Dict:
        """Train model with class weights and proper evaluation"""
    ...
    def run_comprehensive_evaluation(self, model_class, model_name: str, data: Data) -> Dict:
        """Run comprehensive evaluation with class weights and balanced metrics"""
        logger.info(f"Running comprehensive evaluation for {model_name}...")
        
        # Calculate class weights
        class_weights = self.calculate_class_weights(data)
    ...
    def save_results(self, results: Dict):
        ...
        # Save JSON results
        with open(f'results/{results["model_name"]}_fixed_results.json', 'w') as f:
            json.dump(json_results, f, indent=2)
        
        # Save detailed metrics
        metrics_file = f'results/{results["model_name"]}_fixed_metrics.json'
        with open(metrics_file, 'w') as f:
            json.dump(results['average_metrics'], f, indent=2)
        
        logger.info(f"Results saved to results/{results['model_name']}_fixed_results.json")
        logger.info(f"Metrics saved to {metrics_file}")

def main():
    """Main function to run enhanced training with fixes"""
    logger.info("Starting enhanced training with fixes for class imbalance...")
    
    # Initialize trainer
    trainer = EnhancedTrainerFixed(device='cpu')
    ...
    for model_name, model_class in models.items():
        logger.info(f"\n{'='*50}")
        logger.info(f"Training {model_name} (Fixed)")
        logger.info(f"{'='*50}")
    ...
    logger.info(f"\n{'='*80}")
    logger.info("FINAL RESULTS COMPARISON (FIXED)")
    logger.info(f"{'='*80}")
    
    for model_name, results in all_results.items():
        metrics = results['average_metrics']
        logger.info(f"{model_name:12} | F1: {metrics['f1']:.4f} | Balanced Acc: {metrics['balanced_accuracy']:.4f} | "
                   f"ROC-AUC: {metrics['roc_auc']:.4f} | PR-AUC: {metrics['pr_auc']:.4f}")
    
    logger.info("Enhanced training with fixes complete!")
